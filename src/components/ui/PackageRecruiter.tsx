...
const PackageBasic = () => {
  ...
  const [isPackageChoosen, setIsPackageChoosen] = useState(false);

  const handleInfoClick = () => {
    setIsTooltipVisible(!isTooltipVisible);
  };
  const handleChooseClick = () => {
    setIsPackageChoosen(!isPackageChoosen);
  };

  return (
    <div className={`py-10 pl-6 pr-30 mt-10 relative max-w-[600px] info_bg border-bec border-3 ${isPackageChoosen ? "border_is" : "border_not"}`}>
      ...
      <div className={`text-start ${awide.className} leading-[1.3]`}>
        <button className="mt-32 text-center px-12 py-3 text-xl bg-[#5A5A58] text-[#CFCFCF] border-2 border-[#CFCFCF] shadow-[inset_-3px_1px_15px_rgba(0,0,0,0.5)] cursor-pointer
