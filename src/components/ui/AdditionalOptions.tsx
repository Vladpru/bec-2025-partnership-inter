      ...
      <div className="max-w-[1200px] flex flex-col mx-auto">
        {addOptions.map(service => {
          ...
          return (
            <div
              key={service.id}
              className={`relative flex items-center p-3 mb-4 transition-colors info_bg cursor-pointer
                ${isSelected ? 'border_is' : 'border_not'}`}
              onClick={() => {
                if (!isDisabled) onToggleService(service.name);
              }}
            >
              <input
                type="checkbox"
              ...
              {service.available !== 0 && (
                <p className="text-sm font-extralight text-center bg-[#585856] text-becwhite py-1 px-6 mr-5 border-1 border-becwhite max-w-[250px]">
              ...
              <button
                className={`text-center px-7 py-1 text-xl bg-[#5A5A58] text-becwhite border-2 border-[#CFCFCF] 
                  ...
                  transition-all duration-400 ease-out ${awide.className} w-[120px]`}
                onClick={e => {
                  e.stopPropagation();
                  onToggleService(service.name);
                }}
              ...
              <span
                ...
                cursor-pointer hover:scale-125 transition-all ease-in"
                onClick={e => {
                  e.stopPropagation();
